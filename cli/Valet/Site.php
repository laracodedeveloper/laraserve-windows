...
class Site
{
    ...
    {
        $certsPath = LARASERVE_HOME_PATH.'/Certificates';
    ...
    public function secure($url)
    {
        ...
        $this->files->putAsUser(
            LARASERVE_HOME_PATH."/Nginx/$url.conf", $this->buildSecureNginxServer($url)
        );
    }
    ...
    {
        if ($this->files->exists($path = LARASERVE_HOME_PATH."/Nginx/$url.conf")) {
    ...
    public function buildSecureNginxServer($url)
    {
        ...
        return str_replace(
            ['LARASERVE_HOME_PATH', 'LARASERVE_SERVER_PATH', 'LARASERVE_STATIC_PREFIX', 'LARASERVE_SITE', 'LARASERVE_CERT', 'LARASERVE_KEY', 'HOME_PATH'],
            [LARASERVE_HOME_PATH, LARASERVE_SERVER_PATH, LARASERVE_STATIC_PREFIX, $url, $path.'/'.$url.'.crt', $path.'/'.$url.'.key', $_SERVER['HOME']],
            $this->files->get(__DIR__.'/../stubs/secure.laraserve.conf')
        );
    }
    ...
    public function unsecure($url)
    {
        if ($this->files->exists($this->certificatesPath().'/'.$url.'.crt')) {
            $this->files->unlink(LARASERVE_HOME_PATH."/Nginx/$url.conf");
    ...
    {
        return LARASERVE_HOME_PATH.'/Sites';
    }
    ...
    {
        return LARASERVE_HOME_PATH.'/Certificates';
    }
}
